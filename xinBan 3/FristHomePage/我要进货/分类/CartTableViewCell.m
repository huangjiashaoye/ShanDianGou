...
-(void)reloadDataWith:(CartModel*)model
{
    ...
    [self.imageView_cell sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@",model.ProductPic]]];
    
    self.nameLabel.text = model.ProductName;
    self.priceLabel.text = [NSString stringWithFormat:@"￥%@",model.ProductPrice];
    self.dateLabel.text = model.dateStr;
    self.numberLabel.text = [NSString stringWithFormat:@"%ld",(long)model.ProductCount];
    self.sizeLabel.text = model.ProductId;
...
-(void)setupMainView
{
    ...
    self.selectBtn.selected = self.isSelected;
    [self.selectBtn setImage:[UIImage imageNamed:@"NoSelectOne"] forState:UIControlStateNormal];
    [self.selectBtn setImage:[UIImage imageNamed:@"PicSelect"] forState:UIControlStateSelected];
...
//    self.sizeLabel.text = @"尺寸:58*86cm";
    self.sizeLabel.textColor = [UIColor clearColor];
    self.sizeLabel.font = [UIFont systemFontOfSize:0];
...
//    self.priceLabel.text = @"￥100.11";
    ...
    [addBtn setImage:[UIImage imageNamed:@"Add_pic"] forState:UIControlStateNormal];
    [addBtn setImage:[UIImage imageNamed:@"Add_pic"] forState:UIControlStateHighlighted];
    [addBtn addTarget:self action:@selector(addBtnClick) forControlEvents:UIControlEventTouchUpInside];
    [bgView addSubview:addBtn];
    
    //数量减按钮
    UIButton *cutBtn = [UIButton buttonWithType:UIButtonTypeCustom];
    [cutBtn setImage:[UIImage imageNamed:@"Purch_pic"] forState:UIControlStateNormal];
    [cutBtn setImage:[UIImage imageNamed:@"Purch_pic"] forState:UIControlStateHighlighted];
...
#pragma mark - 添加约束
    ...
    [imageBgView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.left.equalTo(self.selectBtn.mas_right).offset(16);
    ...
    [self.imageView_cell mas_makeConstraints:^(MASConstraintMaker *make) {
        make.edges.equalTo(imageBgView).insets(UIEdgeInsetsMake(0.8, 0.8, 0.8, 0.8));
    }];
    
    //商品名
    self.nameLabel.frame = CGRectMake(0.36 * KScreenWidth,10, KScreenWidth * 0.6, 60);
...
//    }];
    ...
    [self.priceLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.top.equalTo(bgView).offset(83);
        make.width.equalTo(self.nameLabel);
    }];
    
    //数量加按钮
    [addBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(bgView).offset(0.52 * KScreenWidth);
        make.bottom.equalTo(bgView).offset(-10);
        make.height.equalTo(@20);
        make.width.equalTo(@20);
    }];
