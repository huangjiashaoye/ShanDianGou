...
+(NSMutableArray *)storeOrders:(NSMutableDictionary *)dictionary OrderData:(NSMutableArray *)orderArray isAdded:(BOOL)added{
    ...
    if (added) {
        ...
        if ([[NSString stringWithFormat:@"%@",dic[@"ProductId"]] isEqualToString:[NSString stringWithFormat:@"%@",dict[@"ProductId"]]]){
                    
        NSInteger count = [dict[@"ProductCount"] integerValue];
       
        [dic setValue:[NSString stringWithFormat:@"%ld",(long)count] forKey:@"ProductCount"];
        ...
        for (int i=0; i<orderArray.count;i++) {
            ...
            NSString *prodcutId_One = [NSString stringWithFormat:@"%@",dic[@"ProductId"]];
            
            NSString *prodcutId_Two = [NSString stringWithFormat:@"%@",dict[@"ProductId"]];
            
            

            if ([prodcutId_One isEqualToString:prodcutId_Two]) {
          
             
                
                if ([dict[@"ProductCount"] integerValue]==0) {
                    [orderArray removeObjectAtIndex:i];
                    break;
                }
                
                if ([dic[@"ProductCount"] integerValue] == 0){
                    [orderArray removeObjectAtIndex:i];
                }else{
                    [dic setValue:[NSString stringWithFormat:@"%@", dict[@"ProductCount"]] forKey:@"ProductCount"];
                }
...
+(NSInteger) CountOthersWithorderData:(NSMutableArray *)ordersArray{
    ...
    for (int i = 0; i< ordersArray.count; i++)
    {
        ...
        count += [dic[@"ProductCount"] integerValue];
    }
...
+(NSMutableArray *)UpdateArray:(NSMutableArray *)dataArray atSelectDictionary:(NSMutableDictionary *)dictionary{
    ...
    for (NSMutableDictionary *dic in dataArray) {
        if (dic[@"ProductId"] == dictionary[@"ProductId"]){
            
            NSInteger count = [dictionary[@"ProductCount"] integerValue];
            [dic setValue:[NSString stringWithFormat:@"%ld",(long)count] forKey:@"ProductCount"];
            break;
        }
    }
